...
class UserController {
  ...
  register = async (req, res) => {
    try {
      ...
      const tasks = [
        {
          ...
          desc: "Read atleast 2 Finance Related Articles",
      ...
      const newUser = new User({ name, email, phone, pfp, password: passwordHash, bio: bio || "", inventory, tasks });
      await newUser.save();
      res.status(200).json({ message: "success" });
    } catch (error) {
      console.log(error);
  ...
  addToInventory = async (req, res) => {
    try {
      ...
      if (type == "coins") {
        ...
        if (user.gems < price) return res.status(400).json({ message: "Insufficient gems" });
        user.gems -= price;
        if (itemType == "avatar") {
          user.inventory.avatar.push(item)
        }
  ...
  removeFromInventory = async (req, res) => {
    try {
      ...
      if (!user) return res.status(404).json({ message: "User does not exist!" });
      if (!user.inventory) user.inventory = [];
      if (itemType == "avatar") user.inventory.avatar = user.inventory.avatar.filter((avatar) => avatar._id != id);
      else if (itemType == "potion") user.inventory.potions = user.inventory.potions.filter((potion) => potion._id != id);
      else if (itemType == "boost") user.inventory.boosts = user.inventory.boosts.filter((boost) => boost._id != id);
      else return res.status(400).json({ message: "Invalid itemType" });
      user.save();
      res.status(200).json({ message: "success" });
    } catch (err) {
  ...
  setTaskStatus = async (req, res) => {
    try {
      ...
      for (let i = 0; i < user.tasks.length; i++) {
        if (user.tasks[i].isCompleted && user.tasks[i].isAdminGenerated && user.tasks[i]._id == id) {
          user.coins += 3;
          user.gaming.exp += 3;
          ...
          if (party) {
            ...
            party.logs.push(`${user.name} completed a task and damaged the Dragon by ${user.gaming.avatar.attack} health points!`);
          }
        };
        if (user.tasks[i].isCompleted && !user.tasks[i].isAdminGenerated && user.tasks[i]._id == id) {
          user.coins += 1;
          ...
          if (party) {
            ...
            console.log("Here");
            party.logs.push(`${user.name} completed a task and damaged the Dragon by ${user.gaming.avatar.attack * 0.7} health points!`);
            if (party.opp.health <= 0) {
  ...
  deleteTask = async (req, res) => {
    try {
      ...
      let temp = [];
      for (let i = 0; i < user.tasks.length; i++) {
        if (user.tasks[i].isAdminGenerated && user.tasks[i]._id == id) return res.status(400).json({ message: "Cannot delete admin generated task" });
      }
      for (let i = 0; i < user.tasks.length; i++) {
  ...
  newDayTaskLoad = async (req, res) => {
    try {
      ...
      for (let i = 0; i < user.tasks.length; i++) {
        ...
        if (todaysDate - taskDate > 120000) {//86400000
      ...
      let decreasedHealth = inCompleteAdminTasks * 5 + inCompleteNonAdminTasks * 3;

      let party;
      if (user.partyId) {
        party = await Party.findOne({ _id: user.partyId });
        if (party.isFighting) {
          decreasedHealth += ((party.opp.attack) / (user.gaming.avatar.defense)) * 0.5;
          decreasedHealth = Math.floor(decreasedHealth);
          party.logs.push(`${user.name} lost ${decreasedHealth} health points to the Dragon!`);
        }
      }
  ...
  setIncome = async (req, res) => {
    try {
      ...
      user.monetaryStats.dailyExpenditure.necessities = ((necessities / 100) * income) / 30;
      user.monetaryStats.dailyExpenditure.wants = ((wants / 100) * income) / 30;
      user.monetaryStats.dailyExpenditure.savings = ((savings / 100) * income) / 30;
